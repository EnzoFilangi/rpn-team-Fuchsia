...
test('test longer expression', (t) => {
  const rpnExpression = '1 2 + 4 +'

  t.equal(compute(rpnExpression), 7)

  t.end()
})

test('test longer expression', (t) => {
  const rpnExpression = '1 2 3 + +'

  t.equal(compute(rpnExpression), 6)
...
test('test longer expression', (t) => {
  ...
  t.equal(compute(rpnExpression), -1)
...
test('test longer expression', (t) => {
  ...
  t.equal(compute(rpnExpression), 1 / 6)
...
test('test longer expression', (t) => {
  ...
  t.equal(compute(rpnExpression), 3 / 2)
